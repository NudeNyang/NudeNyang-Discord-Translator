...
test("GitHub README는 짧은 GIF 다음에 전체 시연 영상을 제공한다", async () => {
  const [quickDemoGif, fullDemo] = await Promise.all([
    stat(new URL("../assets/hero-discord-translation-masked.gif", import.meta.url)),
    stat(new URL("../assets/workflow-discord-translation-masked.mp4", import.meta.url)),
  ]);
  const quickDemoPath = "./landing/assets/hero-discord-translation-masked.gif";
  const fullDemoUrl = "https://github.com/user-attachments/assets/7b6a5900-8ebb-4d89-8466-e7c077826714";

  for (const document of [readme, readmeKo]) {
    const videos = document.match(/^https:\/\/github\.com\/user-attachments\/assets\/[0-9a-f-]+$/gm) ?? [];
    assert.deepEqual(videos, [fullDemoUrl]);
    assert.ok(document.indexOf(quickDemoPath) < document.indexOf(fullDemoUrl));
    assert.doesNotMatch(document, /87bba13d-5ddc-45b1-92a9-dbf694f2e81a/);
    assert.doesNotMatch(document, /Play full demo|전체 시연 재생|full-discord-translation-demo-masked\.mp4/);
  }

  assert.ok(quickDemoGif.size > 100_000, "짧은 모자이크 시연 GIF가 필요합니다.");
  assert.ok(quickDemoGif.size < 10_000_000, "README GIF는 GitHub 첨부 제한 안에 있어야 합니다.");
