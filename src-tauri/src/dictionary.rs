...
fn contextual_sense_score(
    ...
    let (definition, definition_language) = if (displayed_language == context_language
        && !entry.definition.is_empty())
        || entry.original_definition.is_empty()
    {
...
fn contextual_focus_score(definition: &str, focus: &str) -> ContextualSenseScore {
    ...
    for focus_unit in focus_units {
        ...
        for definition_unit in &definition_units {
            ...
            let compact_script_focus = focus_chars >= 2 && !focus_unit.is_ascii();
            if focus_unit == *definition_unit
                || (compact_script_focus && definition_unit.contains(focus_unit))
            {
                best = best.max(60);
            } else if shorter >= 2
                && shorter * 2 >= longer
