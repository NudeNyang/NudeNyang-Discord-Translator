...
`ms`, `nl` 순으로 표시한다. 이 순서는 감지 후보 순서와 분리해 감지 결과에는 영향을 주지 않는다.

## 인터페이스 언어

설정창, 트레이 메뉴, Discord 안의 보내는 메시지·표시 언어 메뉴, 보낸 메시지 원문 전환,
이미지 번역 버튼은 `Auto(System)`과 위 20개 언어를 공통으로 지원한다. 인터페이스 언어와
메시지 번역 대상 언어는 서로 독립된 설정이며, 인터페이스를 바꿔도 메시지 감지 후보나 번역
공급자 선택은 바뀌지 않는다.

`Auto(System)`은 운영체제/웹뷰의 BCP 47 언어 태그를 다음처럼 정규화한다.

- `zh-TW`, `zh-HK`, `zh-MO`, `zh-Hant*` → `zh-Hant`; 나머지 `zh*` → `zh`
- `pt*` → `pt-BR`, `es*` → `es-419`, 이전 인도네시아어 코드 `in*` → `id`
- 나머지는 기본 언어 하위 태그를 제거한 뒤 20개 카탈로그와 비교하고, 지원하지 않으면 영어를 쓴다.

설정창의 정적 문구는 한국어 키와 검수된 영어 의미 기준문을 원본으로 관리한다. 영어·일본어·간체
중국어는 기존 수동 사전을 유지하며 나머지 16개 언어는 `web/ui-locales.mjs`와 Rust 주입 UI용
`web/ui-locales.json`을 한 번에 생성한다. 신규 언어 사전에 없는 정적 문구는 다른 언어와 섞지 않고
영어 기준문으로 되돌린다. 변수 조합형 런타임 진단 문구도 해당 언어 전용 템플릿이 없으면 임의
번역하지 않고 영어 템플릿을 사용한다. 오역 가능성이 있는 자동 조합보다 의미가 확정된 영어 안내를
우선하는 보수적 정책이다.

20개 인터페이스 언어는 설정창의 열·버튼·메뉴 위치를 동일하게 유지하기 위해 문서 레이아웃을
`dir=ltr`로 고정한다. 아랍어 문구와 언어 이름도 다른 언어와 같은 왼쪽 시작선에 두되, 글자 자체의
양방향 텍스트 처리는 브라우저에 맡겨 아랍어 글자 순서가 바뀌지 않게 한다.

언어 항목이 많아졌으므로 표시 언어, 보내는 언어, 인터페이스 언어 선택창과 트레이·Discord 언어
메뉴에 검색창을 둔다. 검색은 대소문자와 라틴 악센트를 무시하며 다음 세 값을 함께 찾는다.
...
CLI·번역 서비스 연결 동작은 번역문 길이에 따라 카드 폭이 흔들리지 않도록 42px 정사각형의 링크·
...
# 16개 생성 사전의 완전성, 한글 잔존, 자리표시자, 보호 토큰, 길이 이상치 검사
