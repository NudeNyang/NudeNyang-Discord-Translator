...
const overrides = {
  en: {
    ...
    "추가 인증이 필요할 때": "When additional verification is required",
    "Discord가 추가 인증 화면을 표시하면 NudeNyang은 입력 충돌을 막기 위해 번역 연결을 잠시 분리합니다. 일반 Discord에서 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택하면 번역을 계속 사용할 수 있습니다.": "When Discord shows additional verification, NudeNyang temporarily detaches the translation connection to avoid input conflicts. Complete verification in standard Discord, then select Reconnect NudeNyang in Settings to continue translating.",
    "Discord 이용 약관": "Discord Terms of Service",
    "Discord 고객센터": "Discord Help Center",
    "Discord에서 추가 인증을 요청하면 어떻게 하나요?": "What should I do when Discord requests additional verification?",
    "인증 호환 모드가 번역 연결을 멈추고 일반 Discord로 전환할 수 있게 안내합니다. 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택해 번역을 다시 시작해 주세요.": "Verification compatibility mode pauses the translation connection and offers a switch to standard Discord. After verification, select Reconnect NudeNyang in Settings to resume translation.",
  ...
  ja: {
    ...
    "추가 인증이 필요할 때": "追加認証が必要なとき",
    "Discord가 추가 인증 화면을 표시하면 NudeNyang은 입력 충돌을 막기 위해 번역 연결을 잠시 분리합니다. 일반 Discord에서 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택하면 번역을 계속 사용할 수 있습니다.": "Discordに追加認証画面が表示されると、NudeNyangは入力の競合を防ぐため翻訳接続を一時的に切り離します。通常のDiscordで認証を完了し、設定画面の「NudeNyangを再接続」を選ぶと翻訳を再開できます。",
    "Discord 이용 약관": "Discord利用規約",
    "Discord 고객센터": "Discordヘルプセンター",
    "Discord에서 추가 인증을 요청하면 어떻게 하나요?": "Discordから追加認証を求められた場合はどうすればよいですか？",
    "인증 호환 모드가 번역 연결을 멈추고 일반 Discord로 전환할 수 있게 안내합니다. 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택해 번역을 다시 시작해 주세요.": "認証互換モードが翻訳接続を停止し、通常のDiscordへ切り替える方法を案内します。認証後、設定画面の「NudeNyangを再接続」を選んで翻訳を再開してください。",
  ...
  zh: {
    ...
    "추가 인증이 필요할 때": "需要额外验证时",
    "Discord가 추가 인증 화면을 표시하면 NudeNyang은 입력 충돌을 막기 위해 번역 연결을 잠시 분리합니다. 일반 Discord에서 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택하면 번역을 계속 사용할 수 있습니다.": "Discord 显示额外验证页面时，NudeNyang 会暂时断开翻译连接，以避免输入冲突。在普通 Discord 中完成验证后，选择设置中的“重新连接 NudeNyang”即可继续翻译。",
    "Discord 이용 약관": "Discord 服务条款",
    "Discord 고객센터": "Discord 帮助中心",
    "Discord에서 추가 인증을 요청하면 어떻게 하나요?": "Discord 要求额外验证时该怎么办？",
    "인증 호환 모드가 번역 연결을 멈추고 일반 Discord로 전환할 수 있게 안내합니다. 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택해 번역을 다시 시작해 주세요.": "验证兼容模式会暂停翻译连接，并引导您切换到普通 Discord。完成验证后，请在设置中选择“重新连接 NudeNyang”以恢复翻译。",
  ...
  "zh-Hant": {
    ...
    "추가 인증이 필요할 때": "需要額外驗證時",
    "Discord가 추가 인증 화면을 표시하면 NudeNyang은 입력 충돌을 막기 위해 번역 연결을 잠시 분리합니다. 일반 Discord에서 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택하면 번역을 계속 사용할 수 있습니다.": "Discord 顯示額外驗證頁面時，NudeNyang 會暫時中斷翻譯連線，以避免輸入衝突。在一般 Discord 中完成驗證後，選擇設定中的「重新連接 NudeNyang」即可繼續翻譯。",
    "Discord 이용 약관": "Discord 服務條款",
    "Discord 고객센터": "Discord 說明中心",
    "Discord에서 추가 인증을 요청하면 어떻게 하나요?": "Discord 要求額外驗證時該怎麼辦？",
    "인증 호환 모드가 번역 연결을 멈추고 일반 Discord로 전환할 수 있게 안내합니다. 인증을 마친 뒤 설정 창의 NudeNyang 다시 연결을 선택해 번역을 다시 시작해 주세요.": "驗證相容模式會暫停翻譯連線，並引導您切換到一般 Discord。完成驗證後，請在設定中選擇「重新連接 NudeNyang」以恢復翻譯。",
