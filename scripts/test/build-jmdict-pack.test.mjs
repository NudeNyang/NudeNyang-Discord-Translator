...
test("JMdict builder rejects the reduced common-only source", () => {
  const directory = mkdtempSync(join(tmpdir(), "nudenyang-jmdict-common-test-"));
  try {
    const input = join(directory, "input.json");
    const output = join(directory, "output.json.gz");
    writeFileSync(input, JSON.stringify({ commonOnly: true, words: [] }));
