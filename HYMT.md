...
설정 화면은 선택한 모델 파일 크기, 권장 여유 RAM, 현재 사용 가능한 RAM을 함께 표시한다.
여유 메모리가 부족하거나 전체 RAM이 12GB 미만이면 `Hy-MT2 1.8B`, `CPU/RAM 전용`,
`로컬 모델 예열 유지 끄기`를 묶은 저사양 권장 설정을 제공한다. 이 값은 실행 가능성을 돕는
보수적인 안내 기준이며, 운영체제와 다른 프로그램이 점유하는 메모리에 따라 실제 성능은 달라진다.
