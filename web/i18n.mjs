const COPY = Object.freeze({
  ...
  "Hy-MT2 1.8B Q4 (로컬·간단한 문장)": ["Hy-MT2 1.8B Q4 (local · simple messages)", "Hy-MT2 1.8B Q4（ローカル・簡単な文向け）", "Hy-MT2 1.8B Q4（本地 · 简单句子）"],
  "Hy-MT2 7B Q4 (로컬·간단한 문장)": ["Hy-MT2 7B Q4 (local · simple messages)", "Hy-MT2 7B Q4（ローカル・簡単な文向け）", "Hy-MT2 7B Q4（本地 · 简单句子）"],
  "TranslateGemma 4B Q4 (실험·간단한 문장)": ["TranslateGemma 4B Q4 (experimental · simple messages)", "TranslateGemma 4B Q4（実験・簡単な文向け）", "TranslateGemma 4B Q4（实验 · 简单句子）"],
  ...
  "표시 번역과 보내는 메시지 통역에서 로컬 모델을 선택하면 하나만 실행되며, 한쪽의 선택을 바꾸면 다른 쪽도 같은 모델로 맞춰집니다.": ["When local models are selected for display and outgoing translation, only one runs at a time. Changing one selection aligns the other to the same model.", "表示翻訳と送信メッセージ通訳でローカルモデルを選択した場合、実行されるのは一つだけです。一方の選択を変更すると、もう一方も同じモデルに揃います。", "显示翻译和发送消息翻译都选择本地模型时，一次只运行一个。更改一侧的选择后，另一侧也会同步为相同模型。"],
  ...
  "처리 위치 안내": ["Where processing happens", "処理場所について", "处理位置说明"],
  "로컬 번역 모델과 이미지 OCR은 이 PC에서 처리됩니다. 외부 서비스를 선택하면 번역할 텍스트만 해당 서비스로 전송됩니다.": ["Local translation models and image OCR are processed on this PC. When you select an external service, only the text to translate is sent to that service.", "ローカル翻訳モデルと画像OCRはこのPC上で処理されます。外部サービスを選択した場合のみ、翻訳するテキストがそのサービスへ送信されます。", "本地翻译模型和图片 OCR 均在此电脑上处理。选择外部服务时，仅会将需要翻译的文本发送到该服务。"],
  ...
  "로컬에 저장된 번역 데이터의 사용량을 관리합니다.": ["Manage the storage used by locally saved translation data.", "ローカルに保存された翻訳データの使用量を管理します。", "管理本地保存的翻译数据占用的空间。"],
