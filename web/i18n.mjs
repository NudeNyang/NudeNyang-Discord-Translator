...
export const COPY = Object.freeze({
  ...
  "선택한 구절 전체와 인터페이스 언어에 없는 사전 뜻은 설정된 번역 모델로 자동 번역될 수 있습니다. 외부 모델을 선택했다면 해당 텍스트가 그 서비스로 전송될 수 있습니다. 주변 문맥은 PC 안에서만 사용합니다.": ["The full selected phrase and dictionary meanings unavailable in the interface language may be translated by the configured model. If an external model is selected, that text may be sent to the service. Nearby context is used only on this PC.", "選択した語句全体とインターフェース言語にない辞書の意味は、設定した翻訳モデルで自動翻訳されることがあります。外部モデルを選択している場合、そのテキストがサービスに送信されることがあります。周辺の文脈はPC内だけで使用します。", "所选短语的全文和界面语言中没有的词典释义可能会由已配置的翻译模型自动翻译。选择外部模型时，这些文本可能会发送到相应服务。周边上下文仅在本机使用。"],
