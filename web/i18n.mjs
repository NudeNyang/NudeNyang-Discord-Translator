...
export const COPY = Object.freeze({
  ...
  "현재 열린 대화의 메시지 본문만 별도로 설치한 NudeNyang Windows 앱에 전달하여 로컬 AI로 번역합니다.": ["Only message bodies from the currently open conversation are sent to the separately installed NudeNyang Windows app for translation with local AI.","現在開いている会話のメッセージ本文のみを、別途インストールしたNudeNyang Windowsアプリに渡し、ローカルAIで翻訳します。","仅将当前打开对话的消息正文传递给另行安装的 NudeNyang Windows 应用，使用本地 AI 翻译。"],
  ...
  "입력 및 전송 기능을 사용하지 않으며, 이름과 연락처를 자동으로 번역하지 않습니다.": ["No message composition or sending features are used, and names and contacts are not translated automatically.","入力や送信機能は使用せず、名前や連絡先を自動翻訳しません。","不使用输入或发送功能，也不会自动翻译姓名和联系人。"],
  ...
  "로컬 AI로 현재 열린 대화의 메시지만 번역합니다. 입력 및 전송 기능은 사용하지 않습니다. 브라우저별 개인정보 안내에 동의해야 사용할 수 있습니다.": ["Translate only messages in the currently open conversation using local AI. No message composition or sending features are used. Consent to the privacy notice is required in each browser.","ローカルAIで現在開いている会話のメッセージのみを翻訳します。入力や送信機能は使用しません。使用するにはブラウザーごとにプライバシー案内への同意が必要です。","仅使用本地 AI 翻译当前打开对话中的消息，不使用输入或发送功能。使用前须在每个浏览器中同意隐私说明。"],
  "설정": ["Settings", "設定", "设置"],
  "사전": ["Dictionary", "辞書", "词典"],
