...
export const COPY = Object.freeze({
  ...
  "앱에서 선택한 번역기를 사용합니다. 로컬 모델은 PC에서 처리합니다. 외부 서비스(ChatGPT·Claude·Gemini·DeepL)를 선택하면 대화가 해당 서비스로 전송되고 그 정책이 적용됩니다.": ["The translator selected in the app is used. Local models process text on your PC. Selecting an external service (ChatGPT, Claude, Gemini or DeepL) sends conversations to that service under its policies.","アプリで選択した翻訳エンジンを使用します。ローカルモデルはPC内で処理します。外部サービス（ChatGPT・Claude・Gemini・DeepL）を選択すると、会話はそのサービスに送信され、そのポリシーが適用されます。","使用应用中选择的翻译引擎。本地模型在您的电脑上处理文本。选择外部服务（ChatGPT、Claude、Gemini 或 DeepL）后，对话将发送至该服务，并适用其政策。"],
  "현재 열린 대화의 본문·링크 미리보기와 현재 Discord 서버의 보이는 채널명을 Windows 앱에 전달합니다.": ["Message bodies and link previews in the open conversation, and visible channel names in the current Discord server, are passed to the Windows app.","開いている会話の本文・リンクプレビューと、現在のDiscordサーバーに表示されているチャンネル名をWindowsアプリに渡します。","当前对话的正文、链接预览以及当前 Discord 服务器中可见的频道名称会传递给 Windows 应用。"],
  "동의한 브라우저에서는 별도 메신저 스위치 없이 앱 설정을 사용합니다.": ["After consent in this browser, messenger translation uses the app settings without a separate messenger switch.","このブラウザーで同意すると、メッセンジャー専用スイッチなしでアプリの設定を使用します。","在此浏览器中同意后，将直接使用应用设置，无需单独的聊天翻译开关。"],
  ...
  "업데이트를 확인하고 있습니다...": ["Checking for updates...", "アップデートを確認しています…", "正在检查更新…"],
  "현재 버전이 최신입니다.": ["The current version is up to date.", "現在のバージョンは最新です。", "当前版本已是最新版本。"],
