...
function nativeMessageActionOverlapState() {
  ...
  const pointWithin = outgoing.match(/function pointWithinPaddedRect\(point, bounds, padding = 10\) \{[\s\S]*?\n  \}/);
  ...
  assert.ok(pointWithin, "Discord 작업 막대 포인터 유지 함수를 찾을 수 있어야 해");
...
test("sent translations restore the exact typed original instead of translating twice", () => {
  ...
  assert.match(outgoing, /if \(!this\.isManagedMessage\(root\)\)/);
