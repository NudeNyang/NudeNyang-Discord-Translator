...
이 문서는 Windows 본체 0.7.4-beta와 웹 확장 0.7.11의 웹 동의 v2·사적 읽기 동의 v5 정책을 설명합니다. 이전 본체나 확장은 지원 범위와 보관 방식이 다를 수 있으며, 사적 읽기의 확대된 범위는 기존 동의로 자동 허용하지 않습니다.
...
- 확장 0.7.9 이상에서 제공하는 설치·연결 도움말은 연결 성공 이력과 안내를 접었는지만 브라우저 로컬 저장소에 추가로 저장합니다. 웹페이지 본문·주소·대화 내용은 포함하지 않습니다. 본체 연결 해제나 사적 읽기 동의를 자동으로 변경하지 않습니다.
...
통합 안내에 동의한 뒤 F4 또는 팝업 상단 토글을 켜면 이 브라우저의 모든 탭과 새 탭에 적용됩니다. 켜짐·꺼짐 상태는 브라우저 로컬 저장소에 유지됩니다. 본체 사용 중지와 사이트별 차단은 우회하지 않습니다. 동의 이전에는 페이지 본문을 전송하지 않습니다.
...
After approval on the unified notice, F4 or the popup switch enables or disables translation across all tabs and new tabs in this browser. This state persists in browser-local storage. App disabling and per-site blocking remain effective. Page text is not transmitted before consent.
...
Input values, editable content, code, amounts, and marked contact/identifier fields are excluded. Public navigation, action labels, associated field descriptions and cookie notices may be translated. Account, login, payment, order and administration screens are restricted to identifiable reading-only UI text, not arbitrary data values. HTML metadata cannot identify or anonymize all personal information, including sensitive information in otherwise permitted content. The extension does not inject into browser-internal pages or local files. The developer does not sell webpage text or addresses, or use them for advertising, tracking, analytics, credit assessment or unrelated purposes.
