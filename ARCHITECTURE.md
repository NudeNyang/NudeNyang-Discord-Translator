...
- 자동 재시작 직후 첫 파이프 응답이 늦으면 Discord를 다시 재시작하지 않고 가디언에서 핸들을 새로
  복제해 한 번 더 연결을 검증한다. 두 단계의 실패 원인은 진단 로그에 함께 남긴다.
...
- 연결할 실행 파일의 정규화 경로, 최초 PID, 가디언 실행 인수와 `https://discord.com` 페이지 대상을
  모두 검증한다. PID 인계는 같은 Discord 설치 경로에서 시작된 메인 프로세스에만 허용한다.
...
공급자별 로그인 창은 하나의 인증 세션을 공유하므로 설치·로그인·연결 해제 작업은 전역에서 한 번에
하나만 허용한다. Windows 앱 설치 관리자의 실패 출력은 비밀 값을 제거한 진단 로그에 남긴다.
