...
Discord DOM 세션에 다시 연결하며, 가디언은 Discord가 종료되면 함께 끝난다. Discord가 시작 중
메인 프로세스 PID를 교체하는 환경에서는 최초 PID가 사라진 뒤 최대 15초 동안 동일한 정규화 실행
경로와 전용 파이프 인수를 가진 후속 프로세스에만 가디언 상태를 인계한다. 가디언은 설치 실행 파일을
잠그지 않는 별도 복사본으로 실행하고 다음 시작 때 종료된 복사본과 상태 파일을 정리한다.
다음 경계를 지킨다.
