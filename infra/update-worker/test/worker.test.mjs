...
const manifest = {
  ...
  installer_object_key: "beta/releases/0.3.0-beta.1/NudeTranslator-Setup.exe",
  platforms: {
    "windows-x86_64": {
      object_key: "beta/releases/0.3.0-beta.1/NudeTranslator-Setup.exe",
...
test("authorized clients receive a newer signed update", async () => {
  ...
  assert.equal(
    body.url,
    "https://updates.example/v1/artifacts/beta/releases/0.3.0-beta.1/NudeTranslator-Setup.exe",
  );
});
...
test("installer links accept a revocable beta code", async () => {
  ...
  assert.equal(response.status, 200);
  assert.match(response.headers.get("Content-Disposition"), /NudeTranslator-Setup\.exe/);
