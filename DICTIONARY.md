...
스크롤한다. 결과에는 표제어, 읽기·발음, 품사, 뜻, 예문, 출처와 라이선스를 표시한다. 운영체제
`speechSynthesis`로 발음을 들을 수 있으며 별도 음성 파일은 내려받지 않는다. 재생 중에는 같은
버튼이 일시정지로 바뀌고 다시 선택하면 이어서 재생한다. 발음 언어는 인터페이스 언어가 아니라
표제어의 원문 언어를 기준으로 정규화하며, 운영체제에 설치된 음성 중 같은 언어와 지역의 음성을
우선 선택한다. WebView가 첫 재생 시 음성 목록을 아직 준비하지 못한 경우에는 목록 변경 이벤트를
기다린 뒤 같은 언어의 음성을 명시적으로 지정한다. 해당 언어 음성을 찾지 못하면 다른 언어의 기본
음성으로 잘못 읽지 않도록 재생을 시작하지 않는다.
