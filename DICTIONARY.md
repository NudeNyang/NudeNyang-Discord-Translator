...
다의어는 선택한 표현을 단독으로 번역하지 않고 `앞 문맥 · 선택 표현 · 뒤 문맥`을 하나의 번역
문맥으로 묶는다. 여기서 얻은 문맥상 표현과 주변 문장, 각 사전 뜻을 인터페이스 언어로 맞춘 뒤
정렬한다. 이 경로는 언어별 예외 목록에 의존하지 않으므로 현재 지원 언어와 앞으로 추가할 언어에
