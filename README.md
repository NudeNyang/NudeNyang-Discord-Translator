...
DeepL은 `DEEPL_API_KEY` 환경 변수를 사용한다. 구독 CLI 엔진은 각 공식 CLI에 미리 로그인해야
한다. 앱은 API 결제용 환경 변수를 구독 CLI 자식 프로세스에서 제거하고, 이미지 픽셀은 외부
서비스로 보내지 않으며 OCR로 추출된 텍스트만 선택한 외부 번역기로 전달한다.
