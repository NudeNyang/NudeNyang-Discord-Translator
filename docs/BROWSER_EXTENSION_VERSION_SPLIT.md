# 공유용 0.7.10과 개발용 0.7.12

Firefox에서 이미 업로드 후 삭제된 0.7.9 번호를 다시 사용할 수 없어, 사용자가 보관한
0.7.9를 **기능 변경 없이 0.7.10으로 재패키징**했다. Chrome·Whale도 같은 번호로 맞춘다.
현재 개발 소스는 이 스냅샷과 다르며, 동적 페이지 성능 수정까지 포함한 개인용 빌드는
**0.7.12**로 올려 구분한다. 0.7.11 제출 자료는 당시 소스 기록으로 보존한다.
...
| 개인용 개발 확장 | 0.7.12 | 현재 개발 소스 | `dist/chromium-personal-extension` |
| 0.7.12 제출 패키지 | 0.7.12 | 현재 제출 소스 | `release/browser-extension/0.7.12-submission` |
...
현재 소스에서 패키징 명령을 실행하면 개발·제출용 0.7.12가 생성된다. **그 결과를
공유용 0.7.10이나 제출용 0.7.11로 이름만 바꾸어 제출하지 않는다.** 현재 제출은
[0.7.12 제출 자료](BROWSER_STORE_SUBMISSION_0.7.12.md)를 따르고, 0.7.11 자료는 당시 기록으로 보존한다. 공유용 검토에는 해당 소스 ZIP에
