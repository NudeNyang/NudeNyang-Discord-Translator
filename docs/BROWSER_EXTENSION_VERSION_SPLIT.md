...
현재 소스에서 개인용 패키징 명령을 실행하면 개발용 0.7.12가 생성된다. **그 결과를
공유용 0.7.10이나 제출용 0.7.11로 이름만 바꾸어 제출하지 않는다.** 0.7.11 제출 이력은
[0.7.11 제출 자료](BROWSER_STORE_SUBMISSION_0.7.11.md)를 따른다. 공유용 검토에는 해당 소스 ZIP에
