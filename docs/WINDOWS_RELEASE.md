...
아직 태그가 생성되지 않은 초안은 GitHub의 태그별 조회에서 404를 반환할 수 있다. 인증된 릴리스 목록에서 버전·빌드 커밋·초안 상태가 일치하는 항목을 찾고 숫자 릴리스 ID로 조회한다. 업로드 후 중단된 기존 초안은 원래 빌드 커밋, 여섯 파일의 크기·서명·해시, 릴리스 노트를 다시 대조한 뒤에만 공개를 재개한다. 이때 배포 스크립트·검증 테스트·배포 문서 외에 제품 소스가 변경됐다면 기존 파일을 새 빌드로 간주하지 않는다.
