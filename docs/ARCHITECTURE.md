...
The fixed outgoing and display-language controls are mounted inside Discord's `#app-mount` and use a base stacking context. A hovered semantic message row and its native action group receive the next two local stacking levels, so the native message toolbar remains visible and clickable above the translator instead of being trapped in a separate lower stacking tree. The controls keep their composer-relative position and do not jump away on hover. Turning outgoing interpretation off also cancels queued DOM requests, clears persistent progress feedback, and restores the typed original if a translated review had already been inserted; late worker results are ignored because their DOM request is no longer pending.
