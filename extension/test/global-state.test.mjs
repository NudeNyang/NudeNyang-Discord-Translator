...
test("범위 확대: 이전 웹·메신저 동의를 메일과 계정 UI에 자동 적용하지 않는다", async () => {
  const previousWeb = 1, previousPrivate = 3;
  const p = setup({ webTranslationConsentVersion: previousWeb, messengerConsentVersion: previousPrivate, webTranslationEnabled: true });
  assert.equal((await p.state.get()).enabled, false);
  const state = await p.state.privacyState();
  assert.equal(state.granted, false);
  assert.equal(state.anyGranted, true, "outdated consent can still be revoked");
...
test("여러 탭의 연속 토글을 직렬화하고 마지막 OFF를 재시작에도 보존한다", async () => {
  const p = setup({ webTranslationConsentVersion: 2, webTranslationEnabled: false });
...
test("저장소 읽기에 실패하면 자동 번역을 허용하지 않는다", async () => {
  const p = setup({ webTranslationEnabled: true, webTranslationConsentVersion: 2 });
