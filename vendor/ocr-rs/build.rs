...
fn extract_zip(archive: &Path, dest_dir: &Path) {
    // Windows ships bsdtar, which extracts zip archives without depending on
    // PowerShell module auto-loading. Some packaged/runtime environments can
    // find Expand-Archive but fail to auto-load Microsoft.PowerShell.Archive.
    if cfg!(target_os = "windows") {
        let status = Command::new("tar")
            .args(["-xf"])
            .arg(archive.to_str().unwrap())
            ...
            .arg(dest_dir.to_str().unwrap())
            .status()
            .expect("Failed to run Windows tar");
        if !status.success() {
            panic!("Failed to extract {}", archive.display());
        }
    } else {
...
fn bind_gen(
    ...
    fs::copy(
        manifest_dir.join("src/mnn_bindings.rs"),
        out_path.join("mnn_bindings.rs"),
    )
    .expect("Couldn't copy checked-in MNN bindings");
}
